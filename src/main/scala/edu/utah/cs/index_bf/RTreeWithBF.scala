package edu.utah.cs.index_bf


import edu.utah.cs.index.Index
import edu.utah.cs.spatial.{LineSegment, MBR, Point, Shape}
import edu.utah.cs.util.{BitArray, BloomFilter, BloomFilterMeta}
...
case class RTreeNode(m_mbr: MBR, m_child: Array[RTreeEntry], isLeaf: Boolean) {
  val bf: Array[Int] = {
    if (isLeaf) {
      val tmp = BitArray.create(BloomFilter.meta.num_bits)
      m_child.foreach(x => BloomFilter.put(tmp, x.asInstanceOf[RTreeLeafEntry].traj_id))
      tmp
    } else {
      m_child.foldLeft(BitArray.create(BloomFilter.meta.num_bits)){
        (a, b) => BitArray.or(a, b.asInstanceOf[RTreeInternalEntry].node.bf)
      }
    }
  }
...
case class RTreeWithBF(root: RTreeNode, bf_meta: BloomFilterMeta) extends Index with Serializable {
  ...
  def antiCircleRangeBF(origin: LineSegment, r: Double): Array[Int] = {
    var ans = BitArray.create(bf_meta.num_bits)
    val st = new mutable.Stack[RTreeNode]()
    if (!origin.cover(root.m_mbr, r)) st.push(root)
    while (st.nonEmpty) {
      val now = st.pop()
      if (origin.minDist(now.m_mbr) > r) ans = BitArray.or(ans, now.bf)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
        ...
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
            if (origin.minDist(shape) > r) BloomFilter.put(ans, traj_id)
        }
      }
    }
    ans
  ...
  def circleRangeBF(origin: LineSegment, r: Double): Array[Int] = {
    var ans = BitArray.create(bf_meta.num_bits)
    val st = new mutable.Stack[RTreeNode]()
    if (root.m_mbr.minDist(origin) <= r && root.m_child.nonEmpty) st.push(root)
    while (st.nonEmpty) {
      val now = st.pop()
      if (origin.cover(now.m_mbr, r)) ans = BitArray.or(ans, now.bf)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
            if (origin.minDist(mbr) <= r) st.push(node)
  ...
  def circleRangeBF(query: Array[LineSegment], r: Double): Array[Int] = {
    ...
    while (st.nonEmpty) {
      val now = st.pop()
      if (checkCovered(now.m_mbr)) ans = BitArray.or(ans, now.bf)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
        ...
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
            if (check(shape)) BloomFilter.put(ans, traj_id)
        }
      }
    }
    ans
  ...
  def antiCircleRangeBF(query: Array[LineSegment], r: Double): Array[Int] = {
    def check(now: Shape) : Boolean = {
      for (i <- query.indices)
        if (now.minDist(query(i)) <= r) return false
      true
    ...
    while (st.nonEmpty) {
      val now = st.pop()
      if (check(now.m_mbr)) ans = BitArray.or(ans, now.bf)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(_, node) =>
            if (!checkCovered(root.m_mbr)) st.push(node)
...
object RTreeWithBF {
  def apply(entries: Array[(LineSegment, Int, Int)], max_entries_per_node: Int, bf_meta: BloomFilterMeta): RTreeWithBF = {
    ...
    new RTreeWithBF(root, bf_meta)
  }

  def applyMBR(entries: Array[(MBR, Int, Int)], max_entries_per_node: Int, bf_meta: BloomFilterMeta): RTreeWithBF = {
