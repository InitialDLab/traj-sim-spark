...
import edu.utah.cs.util.BitMap
import edu.utah.cs.util.BitMapMeta
import edu.utah.cs.util.BitArray
...
case class RTreeNode(m_mbr: MBR, m_child: Array[RTreeEntry], isLeaf: Boolean) {
  val bf: Array[Int] = {
    if (isLeaf) {
      val tmp = BitArray.create(BitMap.meta.num_bits)
      m_child.foreach(x => BitMap.put(tmp, x.asInstanceOf[RTreeLeafEntry].traj_id))
      tmp
    } else {
      m_child.foldLeft(BitArray.create(BitMap.meta.num_bits)){
...
case class RTreeWithBM(root: RTreeNode, bf_meta: BitMapMeta) extends Index with Serializable {
      ...
      if (origin.minDist(now.m_mbr) > r) ans = BitArray.or(ans, now.bf)
        ...
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
            if (origin.minDist(shape) > r) BitMap.put(ans, traj_id)
        }
      }
    }
    ans
  ...
  def circleRangeBF(query: Array[LineSegment], r: Double): Array[Int] = {
    ...
    while (st.nonEmpty) {
      ...
      if (checkCovered(now.m_mbr)) ans = BitArray.or(ans, now.bf)
        ...
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
            if (check(shape)) BitMap.put(ans, traj_id)
        }
      }
    }
    ans
...
object RTreeWithBM {
  def apply(entries: Array[(LineSegment, Int, Int)], max_entries_per_node: Int, bm_meta: BitMapMeta): RTreeWithBM = {
    ...
    new RTreeWithBM(root, bm_meta)
  }

  def applyMBR(entries: Array[(MBR, Int, Int)], max_entries_per_node: Int, bm_meta: BitMapMeta): RTreeWithBM = {
