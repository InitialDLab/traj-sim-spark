package edu.utah.cs.index_rr

import edu.utah.cs.index.Index
...
case class RTreeNode(m_mbr: MBR, m_child: Array[RTreeEntry], isLeaf: Boolean) {
  val rr: RoaringBitmap = {
    if (isLeaf) {
      RoaringBitmap.bitmapOf(m_child.map(x => x.asInstanceOf[RTreeLeafEntry].traj_id): _*)
    } else {
      m_child.foldLeft(new RoaringBitmap()) {
        (a, b) => RoaringBitmap.or(a, b.asInstanceOf[RTreeInternalEntry].node.rr)
      }
    }
  }
...
case class RTreeWithRR(root: RTreeNode) extends Index with Serializable {
  ...
  def antiCircleRangeBF(origin: LineSegment, r: Double): RoaringBitmap = {
    var ans = new RoaringBitmap()
    val st = new mutable.Stack[RTreeNode]()
    if (!origin.cover(root.m_mbr, r)) st.push(root)
    while (st.nonEmpty) {
      val now = st.pop()
      if (origin.minDist(now.m_mbr) > r) ans = RoaringBitmap.or(ans, now.rr)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
            if (!origin.cover(mbr, r)) st.push(node)
        }
      } else {
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
            if (origin.minDist(shape) > r) ans.add(traj_id)
        }
      }
    }
    ans
  ...
  def circleRangeBF(origin: LineSegment, r: Double): RoaringBitmap = {
    var ans = new RoaringBitmap()
    val st = new mutable.Stack[RTreeNode]()
    if (root.m_mbr.minDist(origin) <= r && root.m_child.nonEmpty) st.push(root)
    while (st.nonEmpty) {
      val now = st.pop()
      if (origin.cover(now.m_mbr, r)) ans = RoaringBitmap.or(ans, now.rr)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
            if (origin.minDist(mbr) <= r) st.push(node)
  ...
  def circleRangeBF(query: Array[LineSegment], r: Double): RoaringBitmap = {
    ...
    while (st.nonEmpty) {
      val now = st.pop()
      if (checkCovered(now.m_mbr)) ans = RoaringBitmap.or(ans, now.rr)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(mbr, node) =>
        ...
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
            if (check(shape)) ans.add(traj_id)
        }
      }
    }
    ans
  ...
  def antiCircleRange(input: Array[LineSegment], r: Double): Array[(Shape, Int)] = {
	val query = input.filter(x => x.minDist(root.m_mbr) <= r)
    def check(now: Shape) : Boolean = {
      for (i <- query.indices)
    ...
    while (st.nonEmpty) {
      ...
      if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(m_mbr, node) =>
            if (!checkCovered(m_mbr)) st.push(node)
        }
      } else {
        now.m_child.foreach {
          case RTreeLeafEntry(shape, m_data, _, _) =>
  ...
  def antiCircleRangeBF(input: Array[LineSegment], r: Double): RoaringBitmap = {
    val query = input.filter(x => x.minDist(root.m_mbr) <= r)
    def check(now: Shape) : Boolean = {
      for (i <- query.indices)
        if (now.minDist(query(i)) <= r) return false
    ...
    while (st.nonEmpty) {
      val now = st.pop()
      if (check(now.m_mbr)) ans = RoaringBitmap.or(ans, now.rr)
      else if (!now.isLeaf) {
        now.m_child.foreach {
          case RTreeInternalEntry(_, node) =>
            if (!checkCovered(node.m_mbr)) st.push(node)
        }
      } else {
        now.m_child.foreach {
          case RTreeLeafEntry(shape, _, _, traj_id) =>
...
object RTreeWithRR {
  def apply(entries: Array[(LineSegment, Int, Int)], max_entries_per_node: Int): RTreeWithRR = {
    val dimension = entries(0)._1.start.coord.length
    ...
    def recursiveGroupSegments(entries: Array[(LineSegment, Int, Int)],
                               cur_dim: Int, until_dim: Int): Array[Array[(LineSegment, Int, Int)]] = {
    ...
    grouped.foreach(list => {
      ...
      list.foreach(now => {
        for (i <- 0 until dimension) min(i) = Math.min(min(i), Math.min(now._1.start.coord(i), now._1.end.coord(i)))
        for (i <- 0 until dimension) max(i) = Math.max(max(i), Math.min(now._1.start.coord(i), now._1.end.coord(i)))
      })
    ...
    new RTreeWithRR(root)
  }

  def applyMBR(entries: Array[(MBR, Int, Int)], max_entries_per_node: Int): RTreeWithRR = {
