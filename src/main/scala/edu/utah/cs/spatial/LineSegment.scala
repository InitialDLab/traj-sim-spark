...
case class LineSegment(start: Point, end: Point) extends Shape {
  ...
  val centroid = new Point(start.coord.zip(end.coord).map(x => (x._1 + x._2) / 2.0))
  ...
  def matchDist(other: LineSegment): Double = {
    Math.max(Math.max(start.minDist(other), end.minDist(other)),
      Math.max(other.start.minDist(this), other.end.minDist(this)))
  }
  ...
  def intersects(mbr: MBR): Boolean = {
    ...
    if (mbr.contains(start) && mbr.contains(end)) true
    else if (intersects(mbr.low, Point(Array(mbr.high.coord(0), mbr.low.coord(1))))) true
  ...
  def minDist(mbr: MBR): Double = {
    if (mbr.contains(start) && mbr.contains(end)) return 0.0
  ...
  def cover(mbr: MBR, r: Double): Boolean = {
    if (minDist(mbr.low) > r) false
    else if (minDist(mbr.high) > r) false
    else if (minDist(Point(Array(mbr.low.coord(0), mbr.high.coord(1)))) > r) false
    else if (minDist(Point(Array(mbr.high.coord(0), mbr.low.coord(1)))) > r) false
    else true
  }
  ...
  def length: Double = start.minDist(end)

  override def toString: String = "SEG(" + start.toString + "->" + end.toString + ")"

  def toTSV: String = start.coord(0) + "\t" + start.coord(1) +"\t" + end.coord(0) + "\t" + end.coord(1)
}
