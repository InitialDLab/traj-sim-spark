...
case class MBR(low: Point, high: Point) extends Shape {
  ...
  def union(other: MBR): MBR = {
    val new_low = low.coord.zip(other.low.coord).map(x => Math.min(x._1, x._2))
    val new_high = high.coord.zip(other.high.coord).map(x => Math.max(x._1, x._2))
    MBR(Point(new_low), Point(new_high))
  }
